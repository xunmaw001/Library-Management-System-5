...
<template>
    <div class="main-content">
        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '状态' : ''">
                        <el-select v-model="searchForm.jieyuejiluTypes" placeholder="请选择状态">
                            <el-option label="=-请选择-=" value=""></el-option>
                            <el-option
                               v-for="(item,index) in jieyuejiluTypesSelectSearch"
                               v-bind:key="index"
                    ...
                    <el-form-item :label="contents.inputTitle == 1 ? '书架' : ''">
                        <el-select v-model="searchForm.shujiaTypes" placeholder="请选择书架">
                            <el-option label="=-请选择-=" value=""></el-option>
                            <el-option
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('jieyuejilu','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/tushuguanli/upload/jieyuejiluMuBan.xls"
                        >批量导入借阅记录数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('jieyuejilu','导入导出')"
                                ...
                                :on-success="jieyuejiluUploadSuccess"
                                :on-error="jieyuejiluUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('jieyuejilu','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入借阅记录数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('jieyuejilu','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "jieyuejilu.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="jieyuejiluTypes"
                                      header-align="center"
                                      label="状态">
                        <template slot-scope="scope">
                            {{scope.row.jieyuejiluValue}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                                     ...
                                     label="操作">
                        <template slot-scope="scope">
                            <el-button v-if="isAuth('jieyuejilu','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('jieyuejilu','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="sessionTable == 'yonghu' && scope.row.jieyuejiluTypes == 1" type="primary" icon="el-icon-edit" size="mini" @click="huanshu(scope.row.id)">还书</el-button>
                            <el-button v-if="isAuth('jieyuejilu','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
        ...
        <el-dialog title="借阅数量排行榜" :visible.sync="chartVisiable" width="800">
            <el-date-picker
                    v-model="echartsDate"
                    type="month"
                    placeholder="月">
...
<script>
    ...
    //当前表下拉框搜索条件
              jieyuejiluTypesSelectSearch : [],
            form:{
                id : null,
                tushuId : null,
                yonghuId : null,
                jieyuejiluTypes : null,
                insertTime : null,
                createTime : null,
            },
            dataList: [],
        ...
        methods: {
            chartDialog() {
                let _this = this;
                var dateString = _this.echartsDate.getFullYear().toString() +"-"+ (_this.echartsDate.getMonth() + 1 < 10 ? '0' + (_this.echartsDate.getMonth() + 1) : _this.echartsDate.getMonth() + 1).toString();
                let params = {
                    dateFormat :"%Y-%m" //%Y-%m
                    ,riqi : dateString
                    ,thisTable : {//当前表
                        tableName :"jieyuejilu"//当前表表名
            ...
            getDataList() {
                ...
                if (this.searchForm.jieyuejiluTypes!= '' && this.searchForm.jieyuejiluTypes!= undefined) {
                    params['jieyuejiluTypes'] = this.searchForm.jieyuejiluTypes
                }
                
                params['jieyuejiluDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
                ...
                //填充下拉框选项
                this.$http({
                    url: "dictionary/page?dicCode=jieyuejilu_types&page=1&limit=100",
                    method: "get",
                }).then(({data}) => {
                    if(data && data.code === 0){
                        this.jieyuejiluTypesSelectSearch = data.data.list;
                    }
                });
            },
            //每页数
            ...
            huanshu(id) {
                ...
                this.$confirm(`确定进行还书操作?`, "提示", {
                    confirmButtonText: "确定",
                    cancelButtonText: "取消",
                    type: "warning"
                }).then(() => {
                    this.$http({
                        url: "jieyuejilu/huanshu",
                        method: "post",
                        data: id
                    }).then(({data}) => {
                        if(data && data.code === 0){
            ...
            // 导入功能上传文件成功后调用导入方法
            jieyuejiluUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "jieyuejilu/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
